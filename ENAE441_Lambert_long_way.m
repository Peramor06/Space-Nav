...
if (beta_c/2 <= pi/2)
    b_c= -beta_c; %If beta_c/2 in the first quadrant then b_c long = -b_c
else 
    b_c= beta_c; %If beta_c/2 in the fourth quadrant then b_c long = b_c
end   
...
beta= -beta_ast; %Since going the long way beta= -beta_ast, in radians
